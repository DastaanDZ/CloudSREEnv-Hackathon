...
def generate_hard_task_examples():
    ...
    return [
        ...
        (
            "L2_DB_SME",
            "New message from IC: Repair payment-worker for checkout latency. Correct this: payment-worker is not a real service; task4 target is notification-worker.",
            _gold_json({
                "action_type": "UPDATE_CONFIG",
                "service_id": "notification-worker",
                "memory_limit_mb": 2048,
        ...
        (
            "L2_DB_SME",
            "New message from IC: Restart payment-db to clear noisy-neighbor memory pressure. Correct this: do not remediate payment-db for task4.",
            _gold_json({
                "action_type": "UPDATE_CONFIG",
                "service_id": "notification-worker",
                "memory_limit_mb": 2048,
        ...
        (
            ...
            _gold_json({
                "action_type": "MESSAGE_CHANNEL",
                "target": "IC",
                "message": "Root cause: notification-worker is causing node memory pressure and starving payment-db. It needs UPDATE_CONFIG memory_limit_mb 2048.",
            }),
        ),
        (
            "IC",
            "INITIAL ALERT:\n[SYSTEM ALERT] Checkout latency detected on payment-db.\n"
            "New message from L1_Triage: Root cause: payment-db is memory-stressed by notification-worker. It needs UPDATE_CONFIG on notification-worker with memory_limit_mb 2048.",
            _gold_json({
                "action_type": "MESSAGE_CHANNEL",
                "target": "L2_DB_SME",
                "message": "Set notification-worker memory limit to 2048MB to stop starving payment-db.",
...
def main():
    ...
    sft_args = TrainingArguments(
        ...
        learning_rate=1e-4,
        per_device_train_batch_size=SFT_MICRO_BATCH_SIZE,
