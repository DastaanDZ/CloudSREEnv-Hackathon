...
5. Evaluate raw BASE and SFT adapter behavior on the same five tasks.
6. Save logs and plots for judging.
...
Generated artifacts (typical paths):

- `episode_traces/benchmark_results.json` — from `scripts/evaluate_benchmarks.py`
- `episode_traces/unsloth_training_metrics.json` — from Unsloth SFT callback (if using that path)
- `assets/sft_training_loss.png`, `assets/reward_progress.png` — from `scripts/generate_readme_assets.py`
...
For judges, the key comparison is BASE vs SFT strict evaluation and the artifacts from `scripts/evaluate_benchmarks.py` and `scripts/generate_readme_assets.py`, which show whether supervised training changed how the model acts in the environment.
...
- Colab-compatible training script using Transformers, PEFT LoRA, and optional Unsloth.
- Generated training logs and loss plots.
- Generated BASE vs SFT comparison metrics (and optional plots).
